...
export class CourseService {
  ...
  constructor(
    ...
    private preflectService: PrefcourseService,
  ...
  async submitGA(struct: Struct) {
    ...
    if (pref_lecturers.length) {
      ...
      let preflect_obj: any=[];
      for ( preflect_obj of pref_lecturers) {
        let full_lect_string: string = "";
        full_lect_string += "(";
        let count: number = 2;
        let preflect_string:string;
        for ( preflect_string in preflect_obj) {
          if (count !== 0) {
